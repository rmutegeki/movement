...
# sitdown2 stretching 1

	joint_action = DataConfig.JOINT_0038_1
	self._movement(joint_action, self.joint_gap)

	subprocess.call("sudo python cmd_headaction.py -a 2", shell = True)

	joint_action = DataConfig.JOINT_0138
	self._movement(joint_action, self.joint_gap)

	joint_action = DataConfig.JOINT_0139
	self._movement(joint_action, self.joint_gap)

	joint_action = DataConfig.JOINT_0236     # 0038
	self._movement(joint_action, self.joint_gap)
