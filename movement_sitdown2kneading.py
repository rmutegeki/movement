...
class MoveACT:
	...
	joint_m[1] = DataConfig.JOINT_0090
	joint_m[2] = DataConfig.JOINT_0091
	joint_m[3] = DataConfig.JOINT_0092
	joint_m[4] = DataConfig.JOINT_0093

	self._movement(joint_m[0])
	
	step = random.randint(1,4)
	for cnt in range(step):
	   self._movement(joint_m[1])
	   self._movement(joint_m[2])
	   self._movement(joint_m[1])
	   self._movement(joint_m[3])

	step = random.randint(0,2)
	for cnt in range(step):
	   self._movement(joint_m[4])
	   self._movement(joint_m[1])
    ...
    def _leg_posture_set(self):
        ...
        self._leg_movement([3, 5, 0.5, -1.3, -0.2, 0.7]) # st, en, sleep, servo
        self._leg_movement([3, 5, 0.7, -0.7, -0.25, 1.5]) # st, en, sleep, servo
        self._leg_movement([0, 2, 0.5, 1.3, 0.2, -0.7]) # st, en, sleep, servo
        self._leg_movement([0, 2, 0.7, 0.7, 0.25, -1.5]) # st, en, sleep, servo
